...
$sqll = $conn ->prepare('SELECT id_materia FROM calificacion');
$sqll->execute();
while($materias = $sqll->fetch()){
	$materia = $materias['id_materia'];
}

$sqll = $conn ->prepare('SELECT id_alumno FROM calificacion');
$sqll->execute();
while($alumnos = $sqll->fetch()){
	$alumno = $alumnos['id_alumno'];
}
$sqll = $conn ->prepare('SELECT id_calificacion FROM calificacion');
$sqll->execute();
while($id_cal = $sqll->fetch()){
	$calificacion = $id_cal['id_calificacion'];
}
...
$sql->bindParam(':cali',$calificacion);
